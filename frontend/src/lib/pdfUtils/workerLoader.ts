...
async function tryLoadWorker(pdfjsLib: any) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('PDF.js worker loaded from public directory:', workerUrl);
      return;
    }
  } catch (error) {
    console.warn('Failed to load PDF.js worker from public directory:', error);
  }
  ...
  try {
    ...
    if (response.ok) {
      pdfjsLib.GlobalWorkerOptions.workerSrc = workerUrl;
      console.log('PDF.js worker loaded from _next/static/worker:', workerUrl);
      return;
    }
  } catch (error) {
    console.warn('Failed to load PDF.js worker from _next/static/worker:', error);
  }
  ...
  try {
    ...
    console.warn('Failed to load PDF.js worker via direct ESM import:', error);
  }
