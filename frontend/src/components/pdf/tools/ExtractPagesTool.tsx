...
export default function ExtractPagesTool() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      try {
        const extractedPdfBytes = await pdfUtils.extractPages(file, selectedPages);
        setProgress(80);

        // Create a Blob from the extracted PDF bytes
        ...
        console.error('Client-side extraction failed, falling back to server:', clientError);
        setProgress(20);
      }

      // Fall back to server-side extraction if client-side fails
      ...
      // Call the API to extract pages
      const result = await pdfApi.extractPages(file, selectedPages);
  ...
  const handleDownload = () => {
    if (resultUrl) {
      const fileName = `${file?.name.replace('.pdf', '') || 'extracted'}_pages.pdf`;
        ...
        <>
          ...
          {file && pageCount > 0 && (
            <div>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-medium">Select Pages to Extract</h3>
          ...
          <div className="flex justify-center">
            <Button
              ...
              className={!file || selectedPages.length === 0 || isProcessing ? 'opacity-50 cursor-not-allowed' : ''}
            >
      ...
      {isProcessing && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-900/20">
          ...
          <p className="mt-2 text-blue-700 dark:text-blue-300">
            Please wait while we extract the selected pages. This may take a few moments.
          </p>
          <div className="mt-4">
      ...
      {resultUrl && !isProcessing && (
        <div className="mt-8 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-900 dark:bg-green-900/20">
          ...
          <p className="mt-2 text-green-700 dark:text-green-300">
            The selected pages have been extracted successfully.
          </p>
